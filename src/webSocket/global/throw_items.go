...
func throwItems(user *player.Player, msg Message) {
	...
	if err != nil {
		...
		if newBox {
			go SendMessage(Message{Event: "NewBox", Box: mapBox, X: user.GetSquad().MatherShip.X, Y: user.GetSquad().MatherShip.Y,
				IDMap: user.GetSquad().MatherShip.MapID, NeedCheckView: true})
		} else {
			...
			for _, user := range users {
				...
				if dist < 175 { // что бы содержимое ящика не видили те кто далеко
					go SendMessage(Message{Event: "UpdateBox", IDUserSend: user.GetID(), BoxID: mapBox.ID, Inventory: mapBox.GetStorage(),
						Size: mapBox.CapacitySize, IDMap: user.GetSquad().MatherShip.MapID, NeedCheckView: true, X: mapBox.X, Y: mapBox.Y})
				}
			}
		}
	}
