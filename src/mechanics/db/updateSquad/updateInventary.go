...
func InventorySquad(squad *squad.Squad, tx *sql.Tx) {
	for slotNum, slot := range squad.Inventory {
		...
		if slot.InsertToDB && slot.Item != nil {
			_, err := tx.Exec("INSERT INTO squad_inventory (id_squad, slot, item_type, item_id, quantity) VALUES ($1, $2, $3, $4, $5)",
		...
		if !slot.InsertToDB && slot.Item != nil {
			_, err := tx.Exec("UPDATE squad_inventory SET quantity = $1, item_type = $2, item_id = $3 WHERE id_squad = $4 AND slot = $5",
