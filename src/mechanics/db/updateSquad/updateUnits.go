...
func Units(squad *squad.Squad, tx *sql.Tx) {
	...
	for slot, slotUnit := range units {
		...
		if units[slot].Unit != nil && slotUnit.Unit.ID == 0 { // если ид 0 значит этого юнита создали в програме и его еще нет в бд
			id := 0
			err := tx.QueryRow("INSERT INTO squad_units (id_squad, id_body, slot, q, r, rotate, on_map, action, target, queue_attack, hp, use_equip, power, mother_ship) "+
		...
		if units[slot].Unit != nil && slotUnit.Unit.ID != 0 {
			_, err := tx.Exec(
				"UPDATE squad_units "+
					"SET id_body = $1, q = $2, r = $3, rotate = $4, action = $5, target = $6, queue_attack = $7, hp = $8, on_map = $11, use_equip = $12, power = $13 "+
