...
func User(id int, login string) *player.Player {
	rows, err := dbConnect.GetDBConnect().Query("SELECT id, name, mail, credits, experience_point, training "+
		"FROM users "+
	...
	for rows.Next() {
		...
		err := rows.Scan(&id, &name, &mail, &credits, &experiencePoint, &newUser.Training)
		if err != nil {
			log.Fatal("get user " + err.Error())
		}
