...
func User(id int, login string) *player.Player {
	rows, err := dbConnect.GetDBConnect().Query("SELECT id, name, mail, credits, experience_point, training, last_base_id "+
		"FROM users "+
	...
	for rows.Next() {
		...
		err := rows.Scan(&id, &name, &mail, &credits, &experiencePoint, &newUser.Training, &newUser.LastBaseID)
		if err != nil {
			log.Fatal("get user " + err.Error())
		}
