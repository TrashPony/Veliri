...
func Equip(player player.Player, game *localGame.Game) []*equip.Equip {
	...
	rows, err := dbConnect.GetDBConnect().Query("Select equip.id, type.name, equip.used, type.specification, "+
	...
	for rows.Next() {
		...
		err := rows.Scan(&userEquip.ID, &userEquip.Name, &userEquip.Used, &userEquip.Specification, &userEquip.Applicable, &userEquip.Region)
		if err != nil {
			log.Fatal(err)
		}
